...
const CarpentryPage = () => {
  ...
  const fetchWorkers = async () => {
    ...
    try {
      const response = await axios.get(`${baseURL}/api/getcarpenter/all`, {
...
const submitRating = async (workerId) => {
  ...
  try {
    await axios.post(`${baseURL}/api/ratecarpenter/${workerId}`, { rating: selectedRating });
...
setBox(false);
const { data } = await axios.post(`${baseURL}/api/create-order` , {amount : amount});
...
await axios.post(`${baseURL}/api/add-payment`, {
        ...
        <tbody>
          {workers.map((carpenter, index) => (
            <tr key={index} className="worker card">
              ...
              <td>
                 <img className='navi' src={Navi} alt="" onClick={()=>opencard(carpenter)} />
